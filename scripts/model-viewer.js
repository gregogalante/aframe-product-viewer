...
AFRAME.registerComponent('model-viewer', {
  ...
  update: function () {
    if (this.data.gltfModelSeduta) {
      this.modelElSeduta.setAttribute('gltf-model', this.data.gltfModelSeduta)
    }
    if (this.data.gltfModelSeduta) {
      this.modelElBase.setAttribute('gltf-model', this.data.gltfModelBase)
    }
  },
  ...
  switchTexture: function (model, texture) {
    console.log('Switch texture', model)
    var texture = new THREE.TextureLoader().load('./texture/' + texture + '.jpg', function (texture) {
    ...
    model.getObject3D('mesh').traverse(function(node) {
      if (node.isMesh) {
        console.log(node)
  ...
  initEntities: function () {
    ...
    // These are our glTF model content entities.
    var modelElSeduta = this.modelElSeduta = document.createElement('a-entity')
    modelElSeduta.setAttribute('scale', '0.01 0.01 0.01')
    modelElSeduta.setAttribute('position', '0 0.45 0')
    modelEl.appendChild(modelElSeduta)
    var modelElBase = this.modelElBase = document.createElement('a-entity')
    modelElBase.setAttribute('scale', '0.01 0.01 0.01')
    modelEl.appendChild(modelElBase)
    ...
    // manage click of texture changes
    document.querySelectorAll('#switch-texture-base button').forEach((button) => {
      button.addEventListener('click', () => this.switchTexture(this.modelElBase, button.dataset.texture))
    })
    document.querySelectorAll('#switch-texture-seduta button').forEach((button) => {
      button.addEventListener('click', () => this.switchTexture(this.modelElSeduta, button.dataset.texture))
    })
  },
